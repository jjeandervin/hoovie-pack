...
export class ApiService {
  ...
  saveDog(familyId: string, values: Record<string, string>, photo?: File, dogId?: string): Observable<DogProfile> {
    ...
    return dogId
      ? this.http.put<ApiDog>(`${this.baseUrl}/families/${familyId}/dogs/${dogId}`, form).pipe(map(mapDog))
      : this.http.post<ApiDog>(`${this.baseUrl}/families/${familyId}/dogs`, form).pipe(map(mapDog));
  }
  ...
  savePost(familyId: string, content: string, photos: File[], postId?: string, removedPhotoIds: string[] = []): Observable<Post> {
    ...
    removedPhotoIds.forEach((photoId) => form.append('removedPhotoIds', photoId));
    return postId
      ? this.http.put<ApiPost>(`${this.baseUrl}/posts/${postId}`, form).pipe(map(mapPost))
      : this.http.post<ApiPost>(`${this.baseUrl}/families/${familyId}/posts`, form).pipe(map(mapPost));
  }
